...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($identity, $password, $email, $group, $additional_data = array())
	{
	...
	*/
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
	 ...
	 * @author Ben Edmunds
	 **/
	// public function update_users_group($user_id=false,$data)
	// {
	// 	$this->trigger_events('remove_from_group');
	...
	// 	if( ! empty($data))
	// 	{
	// 			$this->db->update($this->tables['users_groups'], $data, array('user_id' => $user_id));
	...
	**/
	 ...
	 * @author aditya menon
	*/
	public function create_group($id,$group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$data = array('id'=>$id,'name'=>$group_name,'description'=>$group_description);
		...
		$group_id = $this->db->insert($this->tables['groups'], $data);
		//$group_id = $this->db->insert_id($this->tables['groups'] . '$id');
